...
public class ExceptionTranslator extends ResponseEntityExceptionHandler {
	...
	private static final String applicationName = "community";
	...
	public ResponseEntity<Object> handleAnyException(Throwable ex, NativeWebRequest request) {
		ProblemDetailWithCause pdCause = this.wrapAndCustomizeProblem(ex, request);
		return this.handleExceptionInternal((Exception) ex, pdCause, this.buildHeaders(ex), HttpStatusCode.valueOf(pdCause.getStatus()), request);
	}
	...
	protected ResponseEntity<Object> handleExceptionInternal(
		...
		body = body == null ? this.wrapAndCustomizeProblem((Throwable) ex, (NativeWebRequest) request) : body;
		return super.handleExceptionInternal(ex, body, headers, statusCode, request);
	}

	protected ProblemDetailWithCause wrapAndCustomizeProblem(Throwable ex, NativeWebRequest request) {
		return this.customizeProblem(this.getProblemDetailWithCause(ex), ex, request);
	}
	...
	private ProblemDetailWithCause getProblemDetailWithCause(Throwable ex) {
		...
		return ProblemDetailWithCause.ProblemDetailWithCauseBuilder.instance().withStatus(this.toStatus(ex).value()).build();
	}
	...
	protected ProblemDetailWithCause customizeProblem(ProblemDetailWithCause problem, Throwable err, NativeWebRequest request) {
		...
		if (problem.getType() == null || problem.getType().equals(URI.create("about:blank")))
			problem.setType(this.getMappedType(err));

		// higher precedence to Custom/ResponseStatus types
		String title = this.extractTitle(err, problem.getStatus());
		...
		if (problem.getDetail() == null) {
			...
			problem.setDetail(this.getCustomizedErrorDetails(err));
		}

		Map<String, Object> problemProperties = problem.getProperties();
		if (problemProperties == null || !problemProperties.containsKey(MESSAGE_KEY)) problem.setProperty(
				MESSAGE_KEY,
				this.getMappedMessageKey(err) != null ? this.getMappedMessageKey(err) : "error.http." + problem.getStatus()
		);

		if (problemProperties == null || !problemProperties.containsKey(PATH_KEY))
			problem.setProperty(PATH_KEY, this.getPathValue(request));

		if ((err instanceof MethodArgumentNotValidException fieldException) &&
				(problemProperties == null || !problemProperties.containsKey(FIELD_ERRORS_KEY))
		) problem.setProperty(FIELD_ERRORS_KEY, this.getFieldErrors(fieldException));

		problem.setCause(this.buildCause(err.getCause(), request).orElse(null));

		return problem;
	}

	private String extractTitle(Throwable err, int statusCode) {
		return this.getCustomizedTitle(err) != null ? this.getCustomizedTitle(err) : this.extractTitleForResponseStatus(err, statusCode);
	}
	...
	private String extractTitleForResponseStatus(Throwable err, int statusCode) {
		ResponseStatus specialStatus = this.extractResponseStatus(err);
	...
	private HttpStatus toStatus(final Throwable throwable) {
		...
		return Optional
				.ofNullable(this.getMappedStatus(throwable))
				.orElse(
						Optional.ofNullable(this.resolveResponseStatus(throwable)).map(ResponseStatus::value).orElse(HttpStatus.INTERNAL_SERVER_ERROR)
				);
	}

	private ResponseStatus extractResponseStatus(final Throwable throwable) {
		return Optional.ofNullable(this.resolveResponseStatus(throwable)).orElse(null);
	}
	...
	public Optional<ProblemDetailWithCause> buildCause(final Throwable throwable, NativeWebRequest request) {
		if (throwable != null && this.isCasualChainEnabled()) {
			return Optional.of(this.customizeProblem(this.getProblemDetailWithCause(throwable), throwable, request));
		}
	...
	private boolean containsPackageName(String message) {
		...
		return StringUtils.containsAny(message, "org.", "java.", "net.", "jakarta.", "javax.", "com.", "io.", "de.", "com.shsoftvina.community");
	}
